...
#### 📥 Запуск:
- Квесты не должны пересекаться: если активен один квест, второй не запускается
- Проверка: `!isPedestrianCrossingQuestActive && !questCarActive && !isPoliceQuestActive`
- Квест срабатывает многократно: после завершения предыдущего квеста спавнится новая машина через 3-8 секунд
- **Условия запуска**:
  - Скорость полицейского автомобиля на 2 или 3 передаче
  - Неактивен другой квест
...
- Показывает `questCar.currentSpeed`
...
- При аресте:
  ...
  - `questCarActive = false` (сбрасывается в `MapStore.removeQuestCarByIndex()`)
...
#### 📋 Список действий для QA:
- [ ] Quest Cars появляется многократно (каждые 3-8 сек после завершения предыдущего квеста)
