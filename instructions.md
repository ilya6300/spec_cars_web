...
## ⛽ Расход топлива (обновлено)

### Правила расхода:

- Топливо расходуется **при включённом зажигании** (`isIgnitionOn = true`), даже без нажатого газа
- Базовый расход: `0.5` ед./тик (когда газ НЕ нажат)
- Усиленный расход: `1.5` ед./тик (когда газ нажат — `pressGas()`)
- Переключение расхода происходит автоматически через `pressGas()` и `releaseGas()`
- Если `fuel <= 0` — газ блокируется, двигатель продолжает работать (если зажигание включено), но расход прекращается

### Поведение:

1. Включили зажигание → топливо начинает расходовать по 0.5/тик
2. Нажали газ → расход увеличивается до 1.5/тик
3. Отпустили газ → расход возвращается к 0.5/тик
4. Топливо кончилось → газ блокируется, машина тормозит, но двигатель работает
...
- При остановке на красном светофоре **30% шанс** запуска квеста
- Проверка происходит **один раз за каждый цикл красного света** (не в каждом тике)
- Выбирается случайный пешеход из `human1`...`human16`
- Если `pedestrianQuestTriggered = true` — квест НЕ запускается
- Сирена должна быть **выключена** для срабатывания квеста
...
- Остановка на красном светофоре (30% шанс при каждой остановке)
...
- Появляется, когда `enemy=true` машина попадает в диапазон `questCar.positionX ∈ [policeScreenX, policeScreenX + 250]` (т.е. `[30, 280]` — «сравнялась» с полицейским по X + 250px)
- Проверка в `mapStore.checkQuestCarDistance()`
- Кнопка **НЕ показывается** поверх активных модальных квестов (пешеходный переход / полицейский квест), чтобы не перекрывать модалки
- При аресте:
  - `countHelp +1`
...
- [ ] Кнопка "Арестовать" появляется при попадании `enemy=true` машины в диапазон `positionX ∈ [30, 280]` (X полицейского + 250px)
- [ ] Кнопка "Арестовать" НЕ появляется поверх модалок пешеходного/полицейского квеста
...
```javascript
test("Pedestrian quest: red light -> quest modal -> click pedestrian -> siren -> car moves -> fine button -> countHelp increases", async ({
  page,
}) => {
...
```javascript
test("Police quest: click aggro human, arrest, countHelp increases", async ({
  page,
}) => {
...
test("Quest Car relative movement: police speed 80, other car 60", async ({
  page,
}) => {
...
test("Quest Cars non-intersection: only one active at a time", async ({
  page,
}) => {
...
| ------------------ | -------------------------------------------------- | --------------------------------- |
| Пешеходный переход | Красный светофор + 30% шанс                        | `pedestrianQuestTriggered = true` |
...
```javascript
test("Pedestrian quest: red light -> quest modal -> click pedestrian -> siren -> car moves -> fine button", async ({
  page,
}) => {
  await page.goto("http://localhost:5173");
  await page.waitForSelector(".game-viewport", { timeout: 10000 });
...
```javascript
test("Quest Car movement: enemy=false (civilian, relative to police)", async ({
  page,
}) => {
  await page.goto("http://localhost:5173");
  await page.waitForSelector(".game-viewport", { timeout: 10000 });
  ...
  const initialX = await page.$eval(
    '[data-type="car"]',
    (el) => parseInt(el.style.left) || 0,
  );

  await page.waitForTimeout(3000);

  const finalX = await page.$eval(
    '[data-type="car"]',
    (el) => parseInt(el.style.left) || 0,
  );
...
_Файл обновляется Архитектором при добавлении нового функционала_
