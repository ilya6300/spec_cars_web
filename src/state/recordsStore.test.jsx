...
test("RecordsStore: loads persisted records on init", async () => {
  ...
  const { default: recordsStore } = await import("./recordsStore.jsx");
  expect(recordsStore.getRecords(GAME_MODES.FREE)).toEqual([
    { timeSec: 120, km: 1.5, coins: 2 },
  ]);
});

test("RecordsStore: free sort by timeSec desc", async () => {
  const { default: recordsStore } = await import("./recordsStore.jsx");
  recordsStore.addRecord(GAME_MODES.FREE, { timeSec: 60, km: 1, coins: 1 });
  recordsStore.addRecord(GAME_MODES.FREE, { timeSec: 120, km: 1, coins: 1 });
  expect(recordsStore.getRecords(GAME_MODES.FREE)[0].timeSec).toBe(120);
});

test("RecordsStore: free sort tie-breaker km", async () => {
  const { default: recordsStore } = await import("./recordsStore.jsx");
  recordsStore.addRecord(GAME_MODES.FREE, { timeSec: 100, km: 1, coins: 1 });
  recordsStore.addRecord(GAME_MODES.FREE, { timeSec: 100, km: 2, coins: 1 });
  expect(recordsStore.getRecords(GAME_MODES.FREE)[0].km).toBe(2);
});

test("RecordsStore: free sort tie-breaker coins", async () => {
  const { default: recordsStore } = await import("./recordsStore.jsx");
  recordsStore.addRecord(GAME_MODES.FREE, { timeSec: 100, km: 2, coins: 1 });
  recordsStore.addRecord(GAME_MODES.FREE, { timeSec: 100, km: 2, coins: 3 });
  expect(recordsStore.getRecords(GAME_MODES.FREE)[0].coins).toBe(3);
});
